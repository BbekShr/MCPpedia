 ...
 *
 * Separate from `Icon` because these are fill-based logos at fixed geometry,
 * not stroke glyphs: they take no stroke weight and must not be restyled. The
 * GitHub mark was previously pasted into three files and the share marks lived
 * inline in ShareButtons; they are here so there is one copy of each.
 *
 * `google` is multi-colour by definition, so it ignores `currentColor` and
 * carries Google's own brand hexes, which their brand guidelines require.
 */

export type BrandName =
  | 'github' | 'google' | 'npm' | 'pypi'
  | 'x' | 'linkedin' | 'reddit' | 'medium'

const BRAND_PATHS: Record<BrandName, ReactNode> = {
  github: (
    <>
...
export function Brand({
  ...
  return (
    <svg
      ...
      height={size}
      viewBox="0 0 24 24"
      fill={name === 'google' ? undefined : 'currentColor'}
      className={className}
