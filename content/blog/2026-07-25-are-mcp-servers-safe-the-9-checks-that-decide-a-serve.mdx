...
- **Sensitive file references buried in the schema** (−2) — the same file paths appearing somewhere in the JSON schema rather than the description, flagged separately because that's the spot a human reviewer never reads
...
Via [deps.dev](https://deps.dev): does the package exist, does anything depend on it (**more than 10** dependents earns a point), was it updated in the last 180 days? A bloated tree — over 200 related projects — loses a point back.
