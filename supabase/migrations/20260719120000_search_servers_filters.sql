-- Push the min_score / transport / author filters into search_servers.
--
-- These three filters were applied in JS *after* DB-side pagination in
-- app/servers/page.tsx, so a filtered search returned under-filled pages,
-- a wrong hasNextPage, and rows that silently vanished between pages. Adding
-- them as optional params (default null) keeps every existing caller working
-- while letting the RPC paginate over the already-filtered set.
...
create or replace function search_servers(
  ...
  min_score_filter integer default null,
...
begin
  return query
    ...
    where
      ...
      and (min_score_filter is null or coalesce(s.score_total, 0) >= min_score_filter)
      and (transport_filter is null or transport_filter = any(s.transport))
      and (author_filter is null or s.author_type = author_filter)
    order by
