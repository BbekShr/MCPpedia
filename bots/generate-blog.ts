 ...
 * data through the templates in lib/blog-templates.ts. No model, no credential.
 * The model-backed paths below (--prepare/--finalize via the Claude Code action,
 * and runFull via the Anthropic API) are retained for manual dispatch, but the
 * schedule no longer depends on either — see the note on renderMode.
 */
...
async function getNewServers() {
  const { data } = await supabase
    .from('servers')
    .select('slug, name, tagline, categories, github_stars, score_total, tool_count')
...
async function getTrendingServers() {
  const { data } = await supabase
    .from('servers')
    .select('slug, name, tagline, github_stars, tags, score_total, categories, tool_count')
    .eq('is_archived', false)
    .not('tags', 'is', null)
...
// --render: plan exactly as the model-backed modes do, then fill each article
// from a template instead of a model. This is what the schedules run.
//
// The blog was generated by the Claude Code action, billed to a Claude
// subscription via CLAUDE_CODE_OAUTH_TOKEN. On 2026-06-25 that token began
// returning `authentication_error: Invalid bearer token`; every scheduled run
// since died in under two seconds having produced nothing, and two token
// refreshes did not change it. Nine weeks of silence from one expiring
// credential is the failure mode worth designing out, so the scheduled path now
...
async function planArticles(meta: Meta): Promise<ArticlePlan[]> {
  ...
  //
  // Skipped in --render mode. Every other article type is a ranking or a
  // breakdown — a shape a template fills honestly. SEO guides are tutorials and
...
/**
 * --render: plan the article(s), then build each one from a template instead of
 * a model, and write it exactly as the other paths do.
 *
 * The templates emit the same body-plus-```json shape the model was prompted
 * for, so this reuses buildArticleFromResponse and writeArticleAndUpdateMeta
 * unchanged — slug, frontmatter and the .meta.json dedup bookkeeping are the
 * same code that shipped every previous post.
 */
async function runRender() {
  console.log(`=== MCPpedia Blog Generator (${securityOnly ? 'security-only, ' : ''}render) ===`)
  console.log(new Date().toISOString())
  ...
  try {
    ...
    if (plans.length === 0) {
      console.log('Nothing to render — no new alerts / insufficient data.')
      run.setSummary({ mode: 'render', written: 0, reason: 'insufficient data' })
      await run.finish()
      return
    }
    ...
    for (const plan of plans) {
      const response = renderArticle(plan.type, plan.data)
      if (!response) {
        // The planner thought there was enough data and the template disagreed
        // (too few rows to rank, an advisory with no server attached). Not fatal
        // on its own — a sibling article may still be publishable.
        const msg = `${plan.type}: template produced no article from the planned data`
        console.error(`  ${msg}`)
        errors.push(msg)
        continue
      }
      ...
      const article = buildArticleFromResponse(response, plan)
      if (!article) {
        const msg = `${plan.type}: rendered article was empty or too short`
        console.error(`  ${msg}`)
        errors.push(msg)
        continue
      }
    ...
    if (written === 0) {
      // Nothing written and the workflow's commit step will find nothing to
      // push. Surface it rather than exiting green on an empty run.
      run.setSummary({ mode: 'render', written, errors })
      await run.fail(
        errors.length
          ? `All ${errors.length} planned article(s) failed to render: ${errors.join('; ')}`
          : 'No articles rendered',
      )
      return
    }

    run.setSummary({ mode: 'render', articles: plans.map(p => p.type), written, errors })
    await run.finish()
    console.log(`\nDone. ${written} article(s) written${errors.length ? `, ${errors.length} skipped` : ''}.`)
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err)
...
async function main() {
  ...
  if (renderMode) return runRender()
